...
## How do I use it?
   - Add your nagios server under `[nagios]` in `nagios_hosts` inventory
...
```
   - Add any hosts for checks in the ```nagios_hosts``` inventory
...
```
   - Run the playbook
```
ansible-playbook -i inventory/nagios_hosts install/nagios.yml
```
