...
check_timezone() {
    ...
    for zone in $EXPECTED_TIMEZONE
    do
      if [[ "$current_timezone" == $zone ]];then
        export EXPECTED_TIMEZONE=$zone
        break
      else
        continue
      fi
	...
	echo "$ERROR_STATE_LABEL: Current timezone is $current_timezone (expected timezone is one of : $EXPECTED_TIMEZONE)"
	exit $ERROR_STATE
    fi
